...
| `player/PlayerMgr` | 玩家内存管理，无锁（单 goroutine 内使用） |
...
1. **Actor 模型彻底消除锁**：`GameActor` 单 goroutine 处理所有游戏逻辑，`player.PlayerMgr` 无需加锁，设计简洁正确。
...
`guild/guild.go` 和 `activity/activity.go` 仅有包声明，`App.New()` 中也未初始化这两个 PlayerMgr。

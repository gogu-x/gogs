...
// Handler 处理来自 gate 的玩家请求，上下文携带玩家数据。
...
func (c *Context) Tree() tree.Context                   { return c.Services().TreeCtx }
func (c *Context) GetPlayer(uid uint64) *player.Player  { return c.Play.Services().Players.Get(uid) }
func (c *Context) SyncPlayer(uid uint64) *player.Player { return c.Play.Services().Players.Get(uid) }
...
func (c *Context) CastCall(
	...
	c.Tree().RequestCallback(pid, msg, cb)
	return true
}

// CastPID 投递一个普通消息
func (c *Context) CastPID(pid tree.PID, msg interface{}) bool {
	return c.Tree().Send(pid, msg)
}
...
func (c *Context) CastPlayerIdMsg(uid uint64, msg proto.Message) {
	...
	c.Cast(def.GameGate, &ipb.PushToMsg{UID: p.UID, Msg: msg})
}

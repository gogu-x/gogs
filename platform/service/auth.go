...
func Register(db *mongo.Database, req *pb_pf.RegisterReq) (*pb_pf.AuthAck, error) {
	if req.ServerId == 0 {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_PARAM, Msg: "server_id required"}, nil
	}
	ctx, cancel := bg()
	defer cancel()
	acc := &store.Account{}
	...
	if err == nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_USERNAME_EXISTS, Msg: "account already exists"}, nil
	}
	if !errors.Is(err, mongo.ErrNoDocuments) {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_UNKNOWN, Msg: err.Error()}, nil
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(req.Password), bcrypt.MinCost)
	if err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_INTERNAL, Msg: "internal error"}, nil
	}
	uid := store.NextUID()
	...
	if _, err := db.Collection(store.ColAccounts).InsertOne(ctx2, newAcc); err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_INTERNAL, Msg: err.Error()}, nil
	}
	token, err := auth.Sign(uid)
	if err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_INTERNAL, Msg: "sign error"}, nil
	}
	return &pb_pf.AuthAck{Code: pb_common.ErrCode_OK, Uid: uid, Token: token}, nil
}
...
func Login(db *mongo.Database, req *pb_pf.AuthLoginReq) (*pb_pf.AuthAck, error) {
	...
	if err := db.Collection(store.ColAccounts).FindOne(ctx, bson.M{"account": req.Account, "server_id": req.ServerId}).Decode(acc); err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_UNKNOWN, Msg: "user not found"}, nil
	}
	if err := bcrypt.CompareHashAndPassword([]byte(acc.PasswordHash), []byte(req.Password)); err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_WRONG_PASSWORD, Msg: "wrong password"}, nil
	}
	token, err := auth.Sign(acc.UID)
	if err != nil {
		return &pb_pf.AuthAck{Code: pb_common.ErrCode_ERR_INTERNAL, Msg: "sign error"}, nil
	}
	return &pb_pf.AuthAck{Code: pb_common.ErrCode_OK, Uid: acc.UID, Token: token, ServerId: acc.ServerId}, nil
}
...
func GetServerList(db *mongo.Database, req *pb_pf.GetServerListReq) (*pb_pf.ServerListAck, error) {
	if req == nil || req.Account == "" {
		return &pb_pf.ServerListAck{Code: pb_common.ErrCode_ERR_PARAM, Msg: "account required"}, nil
	}
	...
	if err != nil {
		return &pb_pf.ServerListAck{Code: pb_common.ErrCode_ERR_INTERNAL, Msg: err.Error()}, nil
	}
	...
	return &pb_pf.ServerListAck{Code: pb_common.ErrCode_OK, Accounts: accounts}, nil
}
